...
@pytest.mark.parametrize("dir_structure, start_path, expected_root", [
    (['repo/.git', 'repo/subdir/file.txt'], 'repo/subdir', 'repo'),
    (['repo/.git', 'repo/subdir1/subdir2/file.txt'], 'repo/subdir1/subdir2', 'repo'),
    (['repo/subdir/.git', 'repo/subdir/file.txt'], 'repo/subdir', 'repo/subdir'),
    (['repo/subdir/.git', 'repo/subdir/subsubdir/file.txt'], 'repo/subdir/subsubdir', 'repo/subdir'),
])
...
def test_find_repo_root_not_found(tmpdir):
    ...
    with pytest.raises(FileNotFoundError, match="Repository root with .git directory not found."):

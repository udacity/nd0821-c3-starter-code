...
@pytest.mark.parametrize("y, preds, expected_precision, expected_recall, expected_fbeta", [
    ...
    (np.array([1, 0, 1, 0, 1, 0]), np.array([1, 1, 1, 0, 0, 0]),
     precision_score(np.array([1, 0, 1, 0, 1, 0]), np.array([1, 1, 1, 0, 0, 0]), zero_division=1),
     recall_score(np.array([1, 0, 1, 0, 1, 0]), np.array([1, 1, 1, 0, 0, 0]), zero_division=1),
     fbeta_score(np.array([1, 0, 1, 0, 1, 0]), np.array([1, 1, 1, 0, 0, 0]), beta=1, zero_division=1))
])
...
def test_model_can_predict(sample_data):
    ...
    assert predictions.shape[0] == X_train.shape[0], "The number of predictions does not match the number of samples"
...
def test_load_model(tmpdir, sample_model):
    ...
    assert isinstance(loaded_model, RandomForestClassifier), "Loaded model is not of type RandomForestClassifier."
    assert isinstance(loaded_encoder, StandardScaler), "Loaded encoder is not of type StandardScaler."
    assert isinstance(loaded_lb, LabelBinarizer), "Loaded label binarizer is not of type LabelBinarizer."

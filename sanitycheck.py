...
def run_sanity_check(test_dir):
    ...
    print('Please enter the path to the file that contains your test cases\
    ...
    test_functions_for_get = list(filter(lambda x:
                                  inspect.getsource(
                                  getattr(module, x)).find('.get(')
                                  != -1, test_function_names))
    test_functions_for_post = list(filter
                                   (lambda x: inspect.getsource(
                                    getattr(module, x)).find(
                                    '.post(') != -1, test_function_names))
    ...
    if not test_functions_for_post:
        ...
        if len(test_functions_for_post) == 1:
            print(f"[{WARNING_COUNT}]")
            WARNING_COUNT += 1
            print("Only one test case was detected \
                  for the POST() method.")
            print("Please make sure you have two test \

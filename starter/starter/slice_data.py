...
def slice_data(preds: np.ndarray, y_test: np.ndarray, data: pd.DataFrame, feature: str) -> None:
    ...
    with open('slice_output.txt', 'w') as f:
        ...
        for i in range(len(values)):
            f.write(f"Value: {values[i]} \t Recall: {recall[i]} \t Precision: {precision[i]} \t F1_score: {f1_score[i]}\n")
